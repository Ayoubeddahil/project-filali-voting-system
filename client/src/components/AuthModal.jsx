...
import { X, Loader, Mail, Lock, User, Building } from 'lucide-react'
...
export default function AuthModal({ onClose }) {
  ...
  const [emailForm, setEmailForm] = useState({ email: '', password: '', confirmPassword: '', role: 'student', organization: '' })
  const { login } = useAuth()
  ...
  const loadGoogleUsers = async () => {
    try {
      const response = await api.getGoogleUsers()
      setGoogleUsers(response.data.users)
    } catch (error) {
            ...
            <div className="mb-4 p-4 bg-gray-50 rounded-lg space-y-2 max-h-64 overflow-y-auto border border-gray-100">
              ...
              {googleUsers.map((user) => (
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:bg-white transition-colors disabled:opacity-50"
                >
